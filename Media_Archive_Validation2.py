...
from config import archive_paths, filename_validation_rules


# live_archive_path = archive_paths["live_archive_path"]
# static_archive_path = archive_paths["static_archive_path"]

# live_archive_path = r"D:\0_Media-Archive\02_metadata-and-rename\static-archive"
# static_archive_path = r"D:\0_Media-Archive\02_metadata-and-rename\live-archive"
...
def compare_and_rename_files(live_archive_path, static_archive_path):
    live_files = {}
    static_files = {}

    # Get all files in live-archive folder and store their identifiers
    for root, dirs, files in os.walk(live_archive_path):
    ...
    for root, dirs, files in os.walk(static_archive_path):
        for file_name in files:
    ...
    for identifier, file_path in static_files.items():
        if identifier in live_files:
            ...
            new_static_path = os.path.join(static_archive_path, live_base + os.path.splitext(file_path)[-1])
...
def compare_and_rename_files(live_archive_path, static_archive_path):
    ...
    for identifier, file_name in static_files.items():
        if identifier in live_files and live_files[identifier] != file_name:
            ...
            new_static_path = os.path.join(static_archive_path, new_static_name)
            old_static_path = os.path.join(static_archive_path, file_name)
...
def compare_file_counts(live_archive_path, static_archive_path):
    live_files = {}
    static_files = {}
...
def validation_menu(options):
    ...
    while True:
        ...
        if validation_choice == "1":
            ...
            invalid_files = find_invalid_files(live_archive_path, "live-archive", create_csv=False, display_reason=True)
            if invalid_files:
            ...
            invalid_files = find_invalid_files(static_archive_path, "static-archive", create_csv=False, display_reason=True)
            if invalid_files:
            ...
            os.system("cls")
            # Validate Live files
            file_to_validate = list_files(live_archive_path, recursive=True, include=None, exclude=["ini","xmp"])
            ...
            file_to_validate = list_files(static_archive_path, recursive=True, include=None, exclude=["ini","xmp"])
            failed_validations = validate_file_name(file_to_validate)
            for file in failed_validations:
                print(file)
            print(f"{len(failed_validations)} files failed validation")
    ...
    #     static_files_to_delete = find_invalid_files(live_archive_path, "static-archive", create_csv=False, display_reason=True)
...
# compare_folders(live_archive_path, static_archive_path)
