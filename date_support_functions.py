...
def process_metadata_and_people_lookup(create_date, people_list, people_lookup_path):
    ...
    for person in people_list:
        ...
        if person_data:
            ...
            # print(f"THIS IS THE PERSON DATA: CREATE DATE {create_date_str} {person_data}")

            # Check for exact matches without considering time
            if birth_date_str and create_date_str == birth_date_str:
                results["XMP:BirthName"] = person

            else:
                # Convert birth_date_str to 'YYYY-MM-DD' format for comparison
                if birth_date_str:
                    birth_date = datetime.strptime(birth_date_str, '%Y-%m-%d')
                    birth_date_str = birth_date.strftime('%Y-%m-%d')

                    # Check for month and day match
                    if create_date.month == birth_date.month and create_date.day == birth_date.day:
                        results["XMP:BirthdayName"] = person

            # Check for marriage match
            # print(f"marriage_date_str {marriage_date_str}  |  create_date_str {create_date_str}")
