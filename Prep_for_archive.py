...
from detect_file_types_and_convert import convert_image_to_jpg, convert_video_to_h264_mp4


from config import  f_warning, f_info, f_success, f_default, rename_rules_for_archive, live_archive_file_extensions, asset_title_length, guid_length, created_date_fields, modified_date_fields, all_media_files, image_files, video_files, archive_paths
        ...
        for file in static_file_list:
            ...
            if asset_date and title and guid:
                ...
                if file_extension.lower().replace(".", "") in image_files:
                    ...
                    converted_file_name = f"{archive_paths["prep_live_path"]}\\Photos\\{asset_date}_{title}_{guid}.jpg"
                    new_file_name = f"{archive_paths["prep_static_path"]}\\Photos\\{asset_date}_{title}_{guid}{file_extension.lower()}"
                    static_folder_name = f"{archive_paths["prep_static_path"]}\\Photos\\"
                ...
                if file_extension.lower().replace(".", "") in video_files:
                    ...
                    # Create new names for static and live archive files
                    converted_file_name = f"{archive_paths["prep_live_path"]}\\Video\\{asset_date}_{title}_{guid}.mp4"
                    new_file_name = f"{archive_paths["prep_static_path"]}\\Video\\{asset_date}_{title}_{guid}{file_extension.lower()}"
                    static_folder_name = f"{archive_paths["prep_static_path"]}\\Video\\"
