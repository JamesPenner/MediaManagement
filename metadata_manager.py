...
from config import created_date_fields, acdsee_parser, Location_fields, city_fields, state_fields, country_fields, country_code_fields, gps_fields, locality_fields, people_lookup, family_names, filename_validation_rules, keyword_lookup, asset_title_length, keyword_lookup, people_lookup, location_lookup, image_files, video_files, document_files, audio_files, archive_paths
...
class Metadata_Manager:
    ...
    def _filter_unique_values(self, dictionary):
        """
        Filters unique values from a dictionary, keeping only the first occurrence of each unique value.

        Parameters:
            dictionary (dict): A dictionary where keys are unique identifiers and values are the elements to be filtered.

        Returns:
            dict: A dictionary containing only the unique values from the input dictionary, 
                with their corresponding keys preserved.

        Example:
            >>> obj = MyClass()
            >>> input_dict = {'a': 1, 'b': 2, 'c': 1, 'd': 3, 'e': 2}
            >>> obj._filter_unique_values(input_dict)
            {'a': 1, 'b': 2, 'd': 3}
        """
        unique_values = {}
        seen_values = set()
...
# ################################################################
    ...
    def get_media_date_from_filename(self, file):
        metadata_date_data = {}
        filename = os.path.basename(file)
        date_format = "%Y:%m:%d"
        media_date = None
        metadata_accurate_date = None
        media_xdate = filename[:9]
        century = filename[:2]
        century_string = filename[:2] + "00-" + filename[:2] + "99"
        decade = filename[:3] + "0s"
...
# ################################################################
    def filter_date_fields(self, DateType):
        """
        Filters date fields from metadata based on the specified DateType.

        Parameters:
            DateType (str): Specifies the type of date fields to filter. Should be either 'created' or 'modified'.

        Returns:
            dict: A dictionary containing date fields from metadata that match the specified DateType.
                If an invalid DateType is provided, returns an empty dictionary.

        Example:
            >>> obj = MyClass()
            >>> obj.metadata = {'created_date': '2023-01-01', 'modified_on': '2023-02-15', 'status': 'active'}
            >>> obj.filter_date_fields('created')
    ...
    def filter_metadata(self, list_of_metadata_fields: Union[str, List[str]]) -> Dict[str, Optional[Any]]:
        """
        ...
        Args:
            list_of_metadata_fields: A string or list of strings representing the metadata fields to keep in the dictionary.

        Returns:
            A dictionary containing only the specified metadata fields.
        """
        # Ensure the input is a list, even if a single string is provided
        if isinstance(list_of_metadata_fields, str):
            list_of_metadata_fields = [list_of_metadata_fields]
        
        # Use dictionary comprehension for filtering metadata
        filtered_metadata = {
            field: self.metadata.get(field, None) for field in list_of_metadata_fields
        }
...
# ################################################################
    ...
    def extract_person_names_TEST(self, verbose=False):
        try:
            person_dict = {}
            name_to_index = {}  # Dictionary to store name and their assigned number

            # Extract the XMP:RegionName value regardless of it being a string or list
            people_names = self.metadata.get('XMP: RegionsRegionListName', '')
            # if isinstance(people_names, str):
            #     people_names = [people_names]  # Convert single string to list

            if verbose:
                print(f"Extracted names: {people_names}")

            # # First pass: Assign a consistent number for each person in people_names
            # for index, name in enumerate(people_names[:30], start=1):  # Limit to 30 names
            #     number_str = str(index).zfill(2)
            #     name_to_index[name.lower()] = number_str
            #     person_dict[f'XMP:Person{number_str}'] = name  # Assign to XMP:Person fields
                
            #     if verbose:
            #         print(f"Assigned {name} to XMP:Person{number_str}")

            # # Second pass: Iterate through the lookup CSV and match the metadata name
            # with open(people_lookup, mode='r', newline='') as file:
            #     reader = csv.DictReader(file)

            #     for row in reader:
            #         metadata_name = row['Metadata Name'].strip().lower()  # Lowercase for comparison

            #         if verbose:
            #             print(f"Checking for metadata name: {metadata_name}")

            #         # Find the corresponding number from the name_to_index dictionary
            #         for name in people_names[:30]:  # Limit to 30 names
            #             if metadata_name == name.lower():  # Match metadata name to person
            #                 number_str = name_to_index[metadata_name]

            #                 # Assign consistent number to FamilyName, MarriedName, and PersonGroup
            #                 person_dict[f'XMP:FamilyName{number_str}'] = row['Last Name']
            #                 person_dict[f'XMP:MarriedName{number_str}'] = row['Married Names']
            #                 # person_dict[f'XMP:PersonGroup{number_str}'] = row['Metadata Name']
                            
            #                 if verbose:
            #                     print(f"Matched {metadata_name} to {name}:")
            #                     print(f"  Assigned FamilyName{number_str}: {row['Last Name']}")
            #                     print(f"  Assigned MarriedName{number_str}: {row['Married Names']}")
    ...
    def extract_person_names(self, verbose=False):
        try:
            person_dict = {}
            name_to_index = {}  # Dictionary to store name and their assigned number
...
# ################################################################
    ...
    def build_archive_folder_path_based_on_metadata(self, filepath, create_subfolders_from_sets=True):
        """
        Build the archive folder path based on metadata extracted from the given file.

        Args:
            filepath (str): The path to the file for which the archive folder path is to be constructed.
            create_subfolders_from_sets (bool, optional): Flag indicating whether to create subfolders based on sets
                extracted from the file metadata. Defaults to True.

        Returns:
            str or None: The constructed archive folder path if successful, otherwise None.

        Raises:
            Exception: If any error occurs during the process.

        Note:
            This method attempts to extract metadata from the file using ExifTool, then constructs a folder path
            based on various metadata fields such as creation date, set title, set subtitle, etc. If the file extension
            is not recognized, it prompts the user to continue or exit. If any error occurs during the process,
            it returns None and prints a warning message.
        """
        ...
        try:
            ...
            if "live-archive" in filepath:
                path_root = archive_paths.get("live_archive_path")
            elif "static-archive" in filepath:
                path_root = archive_paths.get("static_archive_path")
            else:
                print("Root path cannot be determined. Exiting. Source path must have reference to 'live-archive' or 'static-archive'")
                sys.exit()

            filename_dates = self.get_media_date_from_filename(file_name)
            
            century = f'\\{filename_dates.get("Century", "")}'
            decade = f'\\{filename_dates.get("Decade", "")}'
            year = f'\\{filename_dates.get("Year", "")}'

            if create_subfolders_from_sets:
                try:
                    set_title = self.filter_metadata(["XMP:SetTitle01"])
                    # print("Found Set data:")
                    # print(set_title)
                    set_title = f"\\{set_title.get('XMP:SetTitle01', '')}"
                except:
                    print("No Set data found")
                    pass

                try:
                    set_subtitle = self.filter_metadata(["XMP:SetSubTitle01"])
                    # print(set_subtitle)
                    set_subtitle = f"\\{set_subtitle.get('XMP:SetSubTitle01', '')}"
                except:
                    print("No Subset data found")
                    pass

            try:
                filename_dates = self.get_media_date_from_filename(filepath)
                filename_date = filename_dates.get("AssetDate", "")
            except Exception as e:
                print(f"Warning: Cannot read AssetDate for file {filepath}: {e}")
                return None  # Skip to the next file on error

            try:
                # Get earliest Created Date
                earliest_cdate = self.filter_date_fields("created")
                earliest_cdate = self.get_earliest_date(earliest_cdate)
            except Exception as e:
                print(f"Warning: Cannot read Create Date for file {filepath}: {e}")
                return None  # Skip to the next file on error
            ...
            try:
                ...
                print(f"FilePath: {file_name}  |  File Extension: {file_extension}")
            except Exception as e:
                print(f"Warning: No file extension for file {filepath}: {e}")
                return None  # Skip to the next file on error
            ...
            # Build Path
            return f"{path_root}\\{file_type}\\{century}{decade}{year}{set_title}{set_subtitle}"
        except Exception as e:
            print(f"Error occurred: {e}")
            return None