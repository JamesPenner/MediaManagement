...
def convert_ai_to_jpg(input_file, output_file):
    try:
        subprocess.run([ImageMagick_path, "-density", "300", input_file, output_file], check=True)
...
def convert_document_to_pdf(source_path, target_path):
    ...
    # Check if the source file is already a PDF
    if source_path.lower().endswith('.pdf'):
        try:
            # Simply copy the PDF to the target path
            shutil.copy(source_path, target_path)
            print(f"PDF copied successfully to: {target_path}")
        except Exception as e:
            print(f"Error while copying PDF: {e}")
    else:
        try:
            # Use LibreOffice to convert the document to PDF
            subprocess.run(["libreoffice", "--headless", "--convert-to", "pdf:writer_pdf_Export", source_path, "--outdir", os.path.dirname(target_path)], check=True)
            print(f"Document converted successfully to PDF at: {target_path}")
        except subprocess.CalledProcessError as e:
